...
import com.bot.txcontrol.eum.ErrMsgId;
...
@Slf4j
public class TpMap extends LinkedHashMap<String, String> {
    /** */
    ...
    public void putParam(String key, Object param) {
        ...
        else this.put(key, (String) (param == null ? "" : param));
    }

    /**
     * Returns the value to which the specified key is mapped,or null if this map contains no
     * mapping for the key.<br>
     * More formally, if this map contains a mapping from a key k to a value v such that (key==null
     * ? k==null :key.equals(k)), then this method returns v; otherwiseit returns null. (There can
     * be at<br>
     * most one such mapping.)<br>
     * A return value of null does not necessarilyindicate that the map contains no mapping for the
     * key; it's alsopossible that the map explicitly maps the key to null.The containsKey operation
     * may be<br>
     ...
     * @throws LogicException When Convert Fail But No throw Exception When isSkipError[0] Is True
     */
    //    public TpMap getVo(String msg, boolean... isSkipError) throws LogicException {
    //        try {
    //            if (msg == null || msg.trim().isEmpty()) return this;
    //            return new ObjectMapper().readValue(msg, TpMap.class);
    //        } catch (Exception e) {
    //            StringWriter errors = new StringWriter();
    //            e.printStackTrace(new PrintWriter(errors));
    //            ApLogHelper.error(log, false, LogType.UTIL.getCode(), "{}", errors.toString());
    //            if (isSkipError.length > 0) {
    //                if (isSkipError[0]) return new TpMap();
    //                else throw new LogicException(ErrMsgId.E999.getCode(), "TpMap To Vo 轉換失敗!! ");
    //            } else throw new LogicException(ErrMsgId.E999.getCode(), "TpMap To Vo 轉換失敗!! ");
    //        }
    //    }
    ...
    public String getJsonString(boolean... isSkipError) throws LogicException {
        try {
            ...
            if (isSkipError.length > 0) {
                if (isSkipError[0]) return "";
                else
                    throw new LogicException(
                            ErrMsgId.E999.getCode(), "TpMap to String 轉換失敗 : " + this);
            } else
                throw new LogicException(ErrMsgId.E999.getCode(), "TpMap to String 轉換失敗 : " + this);
        }
    }
}
